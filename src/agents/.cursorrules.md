# NEØ PROTOCOL - ARCHITECTURE CONTEXT (Enhanced)
...
## 💰 REVENUE CRITICAL (PAYS THE BILLS!)

**Priority Order:**

1. **FlowPay** (PIX → Crypto Gateway) - 208 Astro files
...
Lead (Instagram/WhatsApp)
  → FlowCloser qualifies (score 0-100)
  → FlowOFF closes deal
  → Client pays via PIX
...
## ✅ ACTIVE SKILLS (Managed by NEO Skills Manager)

**What's ACTIVE NOW:**

- **FlowCloser**: Instagram DM + WhatsApp lead qualification
- **Notion**: Organize mental chaos → structured plans
...
- CEO Escalável (comparison with smart-ui-mobile)
- NodeMello.run (ARCHIVED - replaced by content-machine local)

## 🏗️ MIO IDENTITY SYSTEM
...
- `neomello/flowcloser-agent`
  ...
  - Tech: IQAI Agent (now called FlowCloser Agent)
  - Status: OPERATIONAL

### 💰 Revenue Critical (HIGH PRIORITY 🔥🔥)
...
### 🏭 Tokenization (org: neo-smart-token-factory)
...
### 🔮 Backend (FUTURE - Not Priority Now)
...
- **Content Machine Local** - Replaces NodeMello.run
...
- CEO Escalável - `/Users/nettomello/CODIGOS/ceo-escalavel-miniapp/`
  - Status: Comparison pending (vs smart-ui-mobile)

## 🎯 CURRENT MISSION: 7-DAY FLOWPAY SHIP
...
- **DAY 7**: **FIRST REAL SALE** (Loop closed! 🎉)
...
## 🏛️ INTEGRATION PATTERN (FlowCloser Model)
...
```
extensions/{project}/
├── integration.json (config)
├── ADR-001.md (why independent)
├── ADR-002.md (integration strategy)
└── schema/ (JSON schemas)
```
...
```
skills/{project}/
├── {action1}.ts (e.g., qualify.ts)
├── {action2}.ts (e.g., dashboard.ts)
└── {action3}.ts (e.g., health.ts)
```
...
┃ CEO Escalável     [----]    ANALYSIS ⏳   ┃
┃ NodeMello.run     [XXXX]    ARCHIVED 🔴   ┃
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
```
...
- ❌ Bring FlowCloser code into Neobot (breaks Railway)
- ❌ Create monorepo structure (defeats orchestration)
- ❌ Skip ADRs (loses architectural context)
- ❌ Use NodeMello.run (it's archived!)
- ❌ Perfect before shipping (ship fast, iterate)
- ❌ Add FlowPay to monorepo (keep independent!)

**Always:**

- ✅ Skills = HTTP clients to independent services
- ✅ Document decisions in ADRs
- ✅ Record actions in Ledger
- ✅ Follow FlowCloser integration model
...
├── neobot/                    (this repo - orchestrator)
├── flowcloser-local/          (FlowCloser agent)
├── flowpay/                   (208 Astro files)
├── evolution-api/             (analysis pending)
└── ceo-escalavel-miniapp/     (comparison pending)
```
...
║                                       ║
║ Code is law. Chaos becomes protocol.  ║
╚═══════════════════════════════════════╝
```
