# NEO BOT — Personal AI Assistant

## 🚀 NEO BOT CLI - Guia Rápido de Comandos
...
* Acesse em: `http://localhost:3000` (após rodar `cd dashboard && node server.js`)
...
**NEO BOT** is a *professional industrial AI assistant* and control plane for the **NEO Protocol**.
It operates on your own infrastructure, providing high-integrity event auditing (Ledger), autonomous operational health monitoring, and seamless integration with communication channels like Telegram and WhatsApp.

Designed for SRE-first environments where auditability and local control are non-negotiable.

[Docs](https://docs.molt.bot) · [Getting Started](https://docs.molt.bot/start/getting-started) · [Updating](https://docs.molt.bot/install/updating)

Preferred setup: run the onboarding wizard (`neobot onboard`). It walks through gateway, workspace, channels, and skills. The CLI wizard is the recommended path and works on **macOS, Linux, and Windows (via WSL2; strongly recommended)**.
...
```bash
npm install -g neobot@latest
# or: pnpm add -g neobot@latest

neobot onboard --install-daemon
```
...
neobot gateway --port 18789 --verbose

# Send a message
neobot message send --to +1234567890 --message "Hello from NEO BOT"

# Talk to the assistant (optionally deliver back to any connected channel: WhatsApp/Telegram/Slack/Discord/Google Chat/Signal/iMessage/BlueBubbles/Microsoft Teams/Matrix/Zalo/Zalo Personal/WebChat)
neobot agent --message "Ship checklist" --thinking high
```

Upgrading? [Updating guide](https://docs.molt.bot/install/updating) (and run `neobot doctor`).
...
Switch channels (git + npm): `neobot update --channel stable|beta|dev`.
...
```bash
git clone https://github.com/neobot/neobot.git
cd neobot
...
Note: `pnpm neobot ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `neobot` binary.

## Security defaults (DM access)

NEO BOT connects to real messaging surfaces. Treat inbound DMs as **untrusted input**.
...
[![Star History Chart](https://api.star-history.com/svg?repos=neobot/neobot&type=date&legend=top-left)](https://www.star-history.com/#neobot/neobot&type=date&legend=top-left)
...
- **[Browser control](https://docs.molt.bot/tools/browser)** — neobot‑managed Chrome/Chromium with CDP control.
...
NEO BOT can auto-configure Tailscale **Serve** (tailnet-only) or **Funnel** (public) while the Gateway stays bound to loopback. Configure `gateway.tailscale.mode`:
...
Minimal `~/.neobot/neobot.json` (model + defaults):
...
NEO BOT was built for **Molty**, a space lobster AI assistant. 🦞
by Peter Steinberger and the community.

- [clawd.me](https://clawd.me)
- [soul.md](https://soul.md)
- [steipete.me](https://steipete.me)
- [@neobot](https://x.com/neobot)
...
Thanks to all NEO BOT contributors:
